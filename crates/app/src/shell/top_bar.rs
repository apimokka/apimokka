...
pub enum ServerState {
    #[default] Stopped,
    Starting, Running, ReloadPending, RestartRequired, Error,
}
impl ServerState {
...
pub fn view(app: &App) -> Element<'_, Message> {
    ...
    let ws_section: Element<Message> = if let Some(snap) = &app.snapshot {
        let chevron = if app.workspace_menu_open { "▲" } else { "▼" };
        let ws_btn = button(
            row![
                text(snap.meta.name.as_str()).size(size::SECTION),
                text(chevron).size(size::CAPTION)
        ...
        row![
            text("·").size(size::SECTION).color(theme::muted(&app.theme())),
            ws_btn,
        ]
        .spacing(space::S2)
        .align_y(Alignment::Center)
    ...
    let reload_btn  = action_btn(app.t(Key::BtnReload), Message::ReloadConfig,
        app.server_state == ServerState::ReloadPending);
    let restart_btn = action_btn(app.t(Key::BtnRestart), Message::RestartServer,
    ...
    let bar = row![
        ...
        server_chip, save_chip,
        Space::new().width(Length::Fill),
        save_btn, reload_btn, restart_btn, server_btn,
    ]
    .spacing(space::S2)
    .align_y(Alignment::Center);
