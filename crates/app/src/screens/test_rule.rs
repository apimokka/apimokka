...
pub fn view(app: &App) -> Element<'_, Message> {
    ...
    let result_el: Element<Message> = match &tr.result {
        None => text(app.t(Key::TestRuleResultHint)).size(size::CAPTION)
        ...
        Some(TestRuleResult::Matched { summary }) => container(
            row![
                ...
                text(summary.as_str()).size(size::CAPTION).color(theme::muted(&app.theme())),
            ]
        ...
        .width(Length::Fill)
        .into(),
        Some(TestRuleResult::NoMatch) => container(
            text(app.t(Key::TestRuleNoMatch)).size(size::BODY),
        )
        .padding(Padding::from([space::S3, space::S4]))
    ...
    let method_btns: Vec<Element<Message>> = methods.iter().map(|m| {
        ...
        column![
            ...
            widgets::field(app.t(Key::TestRulePath),
                text_input("/api/orders", &tr.url_path)
                    ...
                    .width(Length::Fill)
                    .into()),
            widgets::field(app.t(Key::TestRuleHeaders),
                text_input("content-type: application/json", &tr.headers_text)
                    ...
                    .width(Length::Fill)
                    .into()),
            widgets::field(app.t(Key::TestRuleBody),
                text_input("{}", &tr.body)
