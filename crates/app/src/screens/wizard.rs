...
pub fn view(app: &App) -> Element<'_, Message> {
    ...
    // Required fields
    let required = column![
        widgets::field(t(Key::WizardFieldName),
            text_input("payments-mock", &w.name)
                .on_input(Message::WizardSetName).size(size::BODY)
                .padding(Padding::from([space::S2, space::S3])).width(Length::Fill)
                .into()),
        widgets::field(t(Key::WizardFieldFolder),
            text_input("~/projects", &w.folder)
                .on_input(Message::WizardSetFolder).size(size::BODY)
                .padding(Padding::from([space::S2, space::S3])).width(Length::Fill)
                .into()),
    ].spacing(space::S3);

    // Server section
    let server = collapsible(app, t(Key::WizardSectionServer), t(Key::WizardSectionServerHint), 0,
        column![
            row![
                widgets::field(t(Key::WizardFieldHost),
                    text_input("127.0.0.1", &w.host)
                        .on_input(Message::WizardSetHost).size(size::BODY)
                        .padding(Padding::from([space::S2, space::S3])).width(Length::Fill).into()),
                widgets::field(t(Key::WizardFieldPort),
                    text_input("8080", &w.port)
                        .on_input(Message::WizardSetPort).size(size::BODY)
                        .padding(Padding::from([space::S2, space::S3])).width(Length::Fixed(100.0)).into()),
            ].spacing(space::S3).align_y(Alignment::End),
            checkbox(w.tls).label(t(Key::WizardFieldTls))
                .on_toggle(Message::WizardSetTls).size(size::BODY),
        ].spacing(space::S3).into(),
    );

    // Starter section
    let starter = collapsible(app, t(Key::WizardSectionStarter), t(Key::WizardSectionStarterHint), 1,
        column![
            iced::widget::radio(
    ...
    // Trace section
    let trace = collapsible(app, t(Key::WizardSectionTrace), t(Key::WizardSectionTraceHint), 2,
        column![
            checkbox(true).label(t(Key::WizardTraceEnable)).size(size::BODY),
            widgets::field(t(Key::WizardQueueSize),
                text_input("1024", &w.queue_size)
                    .on_input(Message::WizardSetQueueSize).size(size::BODY)
                    .padding(Padding::from([space::S2, space::S3])).width(Length::Fixed(120.0)).into()),
        ].spacing(space::S3).into(),
    );
