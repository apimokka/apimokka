...
pub fn view(app: &App) -> Element<'_, Message> {
    let ws_name = app.snapshot.as_ref().map(|s| s.meta.name.as_str()).unwrap_or("—");
    let ws_path = app.snapshot.as_ref().map(|s| s.meta.path.as_str()).unwrap_or("");

    let current = container(
        column![
            text(app.t(Key::WorkspaceMenuCurrent)).size(size::CAPTION)
                .color(theme::muted(&app.theme())),
            text(ws_name).size(size::BODY),
            text(ws_path).size(size::CAPTION).color(theme::muted(&app.theme())),
        ]
        .spacing(space::S1),
    )
    .padding(Padding::from([space::S3, space::S4]));

    let recent_rows: Vec<Element<Message>> = mock::recent_workspaces().into_iter().map(|ws| {
        let name = ws.name.clone();
        let is_cur = app.snapshot.as_ref().map(|s| s.meta.name == ws.name).unwrap_or(false);
        button(
            container(
                column![
                    text(ws.name).size(size::BODY),
                    text(format!("{} · {}", ws.path, ws.last_opened)).size(size::CAPTION)
                        .color(theme::muted(&app.theme())),
                ]
                .spacing(2.0),
            )
            .padding(Padding::from([space::S2, space::S4]))
            .style(if is_cur { theme::card_selected_style } else { theme::card_style })
            .width(Length::Fill),
        )
    ...
    container(
        column![
            current,
            widgets::divider(),
            scrollable(column(recent_rows).spacing(space::S1)
