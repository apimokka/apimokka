...
script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" 2>/dev/null && pwd -P) || {
    printf 'Matcher oracle self-test: cannot determine script directory\n' >&2
    exit 2
}
checker="$script_dir/check-matcher-oracle.sh"
repository_root=$(cd -- "$script_dir/.." 2>/dev/null && pwd -P) || {
    printf 'Matcher oracle self-test: cannot determine repository root\n' >&2
    exit 2
}

for utility in chmod cp env mkdir mktemp rm sed; do
    command -v "$utility" >/dev/null 2>&1 || {
        printf 'Matcher oracle self-test: required utility not found: %s\n' "$utility" >&2
        exit 2
    }
done

[[ -x "$checker" ]] || {
    printf 'Matcher oracle self-test: checker is not executable: %s\n' "$checker" >&2
    exit 2
}

tmp_parent=${TMPDIR:-"$repository_root/target/tmp"}
mkdir -p -- "$tmp_parent" || exit 2
temp_dir=$(mktemp -d "$tmp_parent/check-matcher-oracle-self-test.XXXXXX") || exit 2

cleanup() {
    if [[ -n "${temp_dir:-}" &&
          "$temp_dir" == "$tmp_parent"/check-matcher-oracle-self-test.* &&
          -d "$temp_dir" ]]; then
        rm -rf -- "$temp_dir"
    else
        printf 'Matcher oracle self-test: refusing unsafe cleanup: %s\n' \
            "${temp_dir:-<empty>}" >&2
        return 2
    fi
}
...
make_fixture "$valid"
write_features \
    "'apimock-routing feature \"default\"'" \
    "'http feature \"default\"' 'http feature \"std\"'"
...
cp -R -- "$valid" "$case_root"
sed -i '/name = "http"/{n;n;s#registry+https://github.com/rust-lang/crates.io-index#git+https://example.invalid/http#;}' "$case_root/Cargo.lock"
expect_result "http source drift" 1 "http source must be" \
    env CARGO="$fake_cargo" "$checker" "$case_root"
...
write_features \
    "'apimock-routing feature \"default\"' 'apimock-routing feature \"experimental\"'" \
    "'http feature \"default\"' 'http feature \"std\"'"
expect_result "routing feature drift" 1 "resolved features must be [default]" \
    env CARGO="$fake_cargo" "$checker" "$valid"

write_features \
    "'apimock-routing feature \"default\"'" \
    "'http feature \"default\"'"
expect_result "http feature drift" 1 "resolved features must be [default, std]" \
    env CARGO="$fake_cargo" "$checker" "$valid"

if (( failures > 0 )); then
    printf 'Matcher oracle self-test: %d/%d checks failed\n' "$failures" "$checks" >&2
    exit 1
fi

printf 'Matcher oracle self-test: %d checks passed\n' "$checks"
