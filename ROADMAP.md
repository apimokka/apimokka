...
| D-5 | **Arrow-key navigation does not scroll the selection into view.** MK-033 line 120 requires navigation to work "correctly with the visible scroll"; index correctness is delivered, auto-scroll is not, so a selection moved past the visible ~320px window becomes invisible. Stated by the implementer rather than found later. | S3 | **M10**, or whenever the palette is next touched. |
| D-6 | **`iced::event::listen_with` ignores capture status by design**, so the application now sees key events a widget already consumed. Correct for `Escape`, which is why it was adopted, and inert for everything else *today*: the only overlap between our global key set and `text_input`'s captured set (`ArrowLeft`, `ArrowRight`, `Backspace`, `Delete`, `End`, `Enter` conditionally, `Escape`, `Home`) is `Escape` itself. **It stops being inert** if `.on_submit` is ever added to the palette's search field — `Enter` would then be captured *and* dispatched globally, executing the selected row twice — or if a future global shortcut lands on any other key in that set. | S3 (latent) | Recorded only. **Re-check whenever the global key set changes.** |
| ~~D-4~~ | ~~The command palette cannot be opened outside a workspace.~~ **Fixed 2026-09-05** in `74e7f75` (M9), option 2: the toggle is inert outside `AppView::Workspace`. | — | **Closed.** |
