...
|---|---|---|---|
| **D-9** | **The command palette's server row is mislabelled when the server is running.** `palette_commands::TABLE` gives it a static `Key::PaletteCmdStartServer` ("Start server") while dispatching `Message::StartStopServer`, a toggle — so with the server running the row reads "Start server" and executing it stops the server. `Command.label` is a plain `Key` field, so the table cannot vary a label by state. **`shell/top_bar.rs:130` does the same thing correctly** (`match app.server_state`), and `Key::PaletteCmdStopServer` already exists for the branch the palette never got: this is the unused half of a conditional nobody wrote, not an orphaned key. Found by the dev team while checking whether a *different* key was orphaned. | **S2** | **Fix before M6** — a participant with the server running is told an action starts something and it stops it, on a surface they are expected to browse. Narrowest form: make `Command.label` a `fn(&App) -> Key`, mirroring the `fn() -> Message` already in the same struct. One field, one row. |
