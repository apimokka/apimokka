...
| 9 | M8 — snora 0.28 adoption | R1 GO |
...
| M8 — snora 0.28 adoption | Blocked | **Code-complete, visual verification outstanding.** MK-058 Phases 1-3 implemented, reviewed and accepted as code-complete 2026-08-04; evidence: `.git-exclude/reviewed/2026-08-04-mk058-snora-0-28-adoption-implementation-review.md`. Commits `265e9b6` (P1), `8caa826` (P2), `2c191aa` (P3), `b0f62a2` (docs), against baseline `cef5e32`. Gate green; CI run `31229529161` green on all six legs. Phase 2 additionally fixed two pre-existing theme-detection bugs, one of which had no test coverage. **Blocking condition:** three acceptance-evidence checks require the live GUI — Phase 1's no-visual-change guarantee, Phase 2's stock-control comparison, and Phase 3's dialog card and `high_contrast_dark` modal dim. **Owner:** project owner, who holds the live-session go-ahead. **Next decision:** run `scripts/ux/probe.sh`, or rule that visual confirmation moves to M6's L3 sessions. All three checks are reconstructible from the recorded SHAs via `git worktree`. M8 does not reach `Complete` until they are captured. |
