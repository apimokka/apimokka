...
| **D-8** | **The command palette lists two rows that do the same thing.** `palette_commands.rs`'s `TABLE` carries both `PaletteCmdSettings` ("Open settings" / 設定を開く) and `PaletteCmdGoSettings` ("Go to Settings" / 設定へ移動), each dispatching `SwitchTab(WorkspaceTab::Settings)`. Typing `settings` returns two identical-behaviour rows. Pre-existing; task 014 correctly left it alone, having been told to move the table and not edit it. | S3 | **Fix before M6** with D-7, task 018 — remove `PaletteCmdSettings`, keeping `PaletteCmdGoSettings` so the three tab-navigation commands read as one set alongside "Go to Routes" and "Go to Trace". |
