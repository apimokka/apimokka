...
## Defects found by M8's capture — scheduled, not deferred

Three defects surfaced on 2026-09-04. **None is a regression from M8**; all are
pre-existing surfaces nobody had executed. Recorded here rather than folded
silently into another milestone.

| # | Defect | Severity | Disposition |
|---|---|---|---|
| D-1 | **`theme::dialog_style` overpaints snora's dialog card** — fill, radius and an 18%-black shadow with **zero border width**, so snora's bordered card never renders. Measured: the card edge is byte-identical at snora 0.28.0 and 0.42.0 despite the `palette.border` repair between them. Over a modal dim the card's fill still separates it; with no dim it does not. | S2 | **M10** — it is a `theme.rs` surface decision, and MK-059 already owns that file. |
| D-2 | **The new-workspace wizard's card has no discernible edge** in `high_contrast_dark` (capture case B2-1). Same root cause as D-1: the wizard is a full screen with no dim behind it. | S2 | **M10**, with D-1. Fixing D-1 fixes this. |
| D-3 | **The wizard's three collapsible section headers render as primary-filled buttons** with near-unreadable hint text and a fill that does not wrap its content. `wizard.rs:179` builds `section_head` with no `.style(...)` and no `.width(...)`, so it falls back to iced's default *primary* style while the hint keeps `theme::muted(...)` — a colour meant for the page background. Not preset-specific; worst in `high_contrast_dark`. **Found by the project owner during the capture, with no case covering it.** | S2 | **M10.** |
| D-4 | **The command palette cannot be opened outside a workspace.** `Ctrl+K` sets `command_palette.open = true`, but `screens::command_palette::view` renders only at `shell/view.rs:87` (`AppView::Workspace`). On Welcome the state changes and nothing draws it. | S3 | **M9** — it is a keyboard-reachability defect and task 014 already owns that surface. |
