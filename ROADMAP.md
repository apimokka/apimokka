...
|---|---|---|---|
| D-11 | **The undo banner advertises a macOS key on every platform.** `shell/view.rs:196` hard-codes `text(" ⌘Z")` beside the Undo button, so a Linux or Windows participant is shown a key their keyboard does not have. It is the one site task 002 missed when it made `accelerator.rs` the single source of accelerator notation: `accelerator::display(Accelerator::Undo)` already returns `Ctrl+Z` off macOS, and the palette calls it. **The control itself is correct** — the button works and `Ctrl+Z` is bound — so this misinforms rather than misleads into the wrong action, which is why it is not D-9's severity. Found 2026-09-09 while checking whether M6's own "platform-conditional copy" scope item was still open. | S3 | **Fix before M6** — the undo banner is the surface a participant meets immediately after their first reversible action, and MK-056 rates confusing copy as a finding. One call site; no new mechanism. Task 020. |
