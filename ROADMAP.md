...
| 9 | M8 — snora adoption (0.28, then 0.29) | R1 GO |
| 10 | M6 — UX acceptance evidence | R1 GO, and M8 before its live runs |
| 11 | R2 — Integration readiness | M5, M8 and M6 |
...
| M8 — snora adoption (0.28, then 0.29) | Blocked | **Code-complete at 0.28.0, one version bump and one human pass outstanding.** MK-058 Phases 1-3 implemented, reviewed and accepted as code-complete 2026-08-04; evidence: `.git-exclude/reviewed/2026-08-04-mk058-snora-0-28-adoption-implementation-review.md`. Commits `265e9b6` (P1), `8caa826` (P2), `2c191aa` (P3), `b0f62a2` (docs), against baseline `cef5e32`. Gate green; CI run `31229529161` green on all six legs. Phase 2 additionally fixed two pre-existing theme-detection bugs, one of which had no test coverage. **Blocking condition:** the acceptance evidence requires the live GUI, and `scripts/ux/probe.sh` returned `PROBE_RESULT=incapable` — the window is a native-Wayland surface invisible to `xdotool`, and niri's IPC exposes no input injection, so nothing can drive the app except a person. **Remaining path, in order:** (1) dev-team task 011 bumps snora to 0.29.0 — one line, nothing to migrate, appearance-neutral by semantic diff of the two tags; (2) owner task 001, roughly 10 minutes at the machine, captures Phase 2 and Phase 3 evidence at 0.29.0. Sequencing reversed 2026-08-15 on evidence from snora's migration guide; reasoning in MK-058 §7. **Owner:** dev team for (1), project owner for (2). No participant recruitment required — this is not a UX session. |
