...
|---|---|---|---|
| D-10 | **No palette route to locale switching.** `Key::PaletteCmdLocale` ("Change locale") exists and appears in no table row. Nothing is broken — EN and JA are reachable from Settings — so this is an omission or a deliberate scope choice. | S3 | **Record only.** Whether the palette should carry it is a product question, not one to settle under M6 time pressure. |
